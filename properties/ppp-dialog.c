...
static const char *ppp_keys[] = {NM_L2TP_KEY_REFUSE_EAP,
...
GHashTable *
ppp_dialog_new_hash_from_connection(NMConnection *connection, GError **error)
{
    GHashTable *  hash;
    NMSettingVpn *s_vpn;
...
static void
auth_methods_setup(GtkBuilder *builder, GHashTable *hash, gboolean ipsec_enabled)
{
    GtkWidget *            widget;
    GtkListStore *         store;
    GtkTreeIter            iter;
    ...
    gboolean               mschap_state  = TRUE;
    gboolean               mschap2_state = TRUE;
    gboolean               has_ipsec_enabled = FALSE;
    ...
    if (ipsec_enabled) {
        has_ipsec_enabled = TRUE;
        use_mppe      = FALSE;
    }

    g_object_set_data(G_OBJECT(builder), "ipsec-enabled", GINT_TO_POINTER(has_ipsec_enabled));
    ...
    offset = gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(widget),
                                                         -1,
                                                         "",
                                                         GTK_CELL_RENDERER(check_renderer),
                                                         "active",
                                                         COL_VALUE,
                                                         "sensitive",
                                                         COL_SENSITIVE,
...
GtkWidget *
ppp_dialog_new(GHashTable *hash, const char *authtype, gboolean ipsec_enabled)
{
    GtkBuilder *builder;
    ...
    auth_methods_setup(builder, hash, ipsec_enabled);
...
GHashTable *
ppp_dialog_new_hash_from_dialog(GtkWidget *dialog, gboolean ipsec_enabled, GError **error)
{
    GHashTable *  hash;
    GtkWidget *   widget;
    GtkBuilder *  builder;
    ...
    if (g_strcmp0(authtype, NM_L2TP_AUTHTYPE_PASSWORD) == 0) {
        ...
        while (valid) {
            ...
            case TAG_MSCHAPV2:
                if (!allowed)
                    g_hash_table_insert(hash,
                                        g_strdup(NM_L2TP_KEY_REFUSE_MSCHAPV2),
                                        g_strdup("yes"));
                break;
            case TAG_EAP:
                if (!allowed)
    ...
    g_hash_table_insert(hash, g_strdup(NM_L2TP_KEY_MTU), g_strdup_printf("%d", mtu_num));

    widget  = GTK_WIDGET(gtk_builder_get_object(builder, "ppp_mru_spinbutton"));
    mru_num = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(widget));
    g_hash_table_insert(hash, g_strdup(NM_L2TP_KEY_MRU), g_strdup_printf("%d", mru_num));
